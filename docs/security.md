...
1. **No secret reaches the database.** Every entry above is a *process input*. Secrets do not appear in any Prisma model, audit log, debug log, or `/hub/diagnostics` response. The output pipeline (§3) enforces this at the HTTP boundary as defense-in-depth.
...
4. **The tenant id is `session.activeOrganizationId` only** — set via Better-Auth `POST /api/auth/organization/set-active`. Stray `X-Tenant-Id` headers are ignored on every path. The resolver lives in `src/core/multi-tenancy/resolve-request-tenant.ts` and is exercised by `tests/stories/resolve-request-tenant.story.test.ts` and `tests/stories/ability-middleware-ignores-tenant-header.story.test.ts`. Unauthenticated routes (`@Public()` handlers, health probes) run without a tenant GUC.
5. **Tenant exemptions are an allowlist.** `tenant-guard.ts` `EXEMPT_PREFIXES` / `EXEMPT_EXACT` paths skip the GUC set (e.g. `/health/*`, `/api/auth/*`, `/hub/*`). Adding to the allowlist is a deliberate cross-cutting decision; for a single route prefer `@Public("<reason>")`.
