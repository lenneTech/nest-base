...
export function planVolumeCollisionCheck(input: VolumeCollisionInput): VolumeCollisionPlan {
  ...
  const message = [
    ...
    "",
    "Recovery (pick one):",
    "  - Wipe the volume in place — the existing `.env`'s password",
    "    initialises the freshly-recreated volume on first boot, so",
    "    you do not need to delete `.env`:",
    "      docker compose down -v && docker compose up -d postgres",
    "      bun run prisma:migrate",
    "    (DESTRUCTIVE — drops the prior data.)",
