...
const RENAMED_FILE_PATHS = ['package.json', 'README.md', 'portless.yml', 'docker-compose.yml'] as const;
...
export function planProjectRename(input: ProjectRenameInput): ProjectRenamePlan {
  ...
  return {
    ...
    oldSlug,
    files: [
      { path: 'package.json', before: files['package.json'], after: rewritePackageJson(files['package.json'], oldLong, newName) },
      { path: 'README.md', before: files['README.md'], after: rewriteReadme(files['README.md'], oldLong, newName) },
      { path: 'portless.yml', before: files['portless.yml'], after: rewritePortless(files['portless.yml'], oldSlug, newName) },
      { path: 'docker-compose.yml', before: files['docker-compose.yml'], after: rewriteCompose(files['docker-compose.yml'], oldSlug, newName) },
    ],
  };
}
...
function readPortlessProject(text: string): string {
  ...
  if (!match) throw new Error('project-rename: portless.yml must contain a top-level `project:` key');
  return match[1]!;
}
...
function rewriteReadme(text: string, oldLong: string, newName: string): string {
  ...
  return text.replace(
    new RegExp(`^#\\s+${escapeRegex(oldLong)}\\b`, 'm'),
    `# ${newName}`,
  );
}
...
function rewriteCompose(text: string, oldSlug: string, newName: string): string {
  let out = text.replace(
    new RegExp(`^name:\\s*${escapeRegex(oldSlug)}$`, 'm'),
    `name: ${newName}`,
  );
