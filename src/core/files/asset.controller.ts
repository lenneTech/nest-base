...
export class AssetController {
  ...
  async get(
    ...
    // MAJ-3: enforce the ?expires= timestamp generated by signUrl(). The two
    // storage adapters (local + postgres) append `?expires=<unix-seconds>` to
    // signed URLs but never verified it server-side — links never expired.
    // When present and in the past, return 410 Gone so clients know the link
    // is permanently invalid (not a temporary 404).
