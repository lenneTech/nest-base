...
function PendingTab({ data }: { data: MigrationsStatus }): ReactNode {
  ...
  const applyOne = useMutation({
    mutationFn: (name: string) =>
      postJson<{ success: boolean; stderr: string }>("/hub/migrations/apply-one", { name }),
    onSettled: refresh,
  });
  const dryRun = useMutation({
    mutationFn: (name: string) =>
      postJson<{ success: boolean; error?: string }>("/hub/migrations/dry-run", { name }),
  });

  const loadPreview = async (name: string) => {
    const result = await fetchJson<{ name: string; sql: string }>(
      `/hub/migrations/preview/${encodeURIComponent(name)}`,
    );
    setPreview(result);
  };
...
function CreateNewTab(): ReactNode {
  ...
  const apply = useMutation({
    mutationFn: (folder: string) =>
      postJson<{ success: boolean }>("/hub/migrations/apply-draft", { name: folder }),
    onSettled: () => {
      setCreated(null);
      setName("");
      refresh();
  ...
  const discard = useMutation({
    mutationFn: (folder: string) =>
      fetch(`/hub/migrations/draft/${encodeURIComponent(folder)}`, { method: "DELETE" }).then(
        (r) => {
