...
export function TracesPage(): ReactNode {
  const initial = useQuery({
    queryKey: ["dev", "traces"],
    queryFn: () => fetchJson<TracesResponse>(`/api/dev/traces.json?limit=${INITIAL_ROW_CAP}`),
  });
...
function TracesBody({ initial }: { initial: TracesResponse }): ReactNode {
  ...
  useEffect(() => {
    ...
    async function tick(): Promise<void> {
      try {
        const next = await fetchJson<TracesResponse>(`/api/dev/traces.json?since=${cursor}`);
        if (cancelled) return;
    ...
    if (!drillCache[reqId]) {
      try {
        const r = await fetchJson<QueriesResponse>(
          `/api/dev/queries.json?requestId=${encodeURIComponent(reqId)}`,
        );
