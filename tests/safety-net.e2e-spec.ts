...
describe('Output-Pipeline · Safety-Net (mode-driven)', () => {
  describe('mode=mask', () => {
    it('replaces secret values with `[redacted]` recursively', () => {
      const out = applySafetyNet({ user: { password: 'p', token: 't', email: 'a@x.com' } }, { mode: 'mask' });
      expect(out).toEqual({ user: { password: '[redacted]', token: '[redacted]', email: 'a@x.com' } });
    });
  ...
  describe('mode=throw', () => {
    it('throws SafetyNetViolationError on the first secret hit', () => {
      expect(() => applySafetyNet({ password: 'p' }, { mode: 'throw' })).toThrow(SafetyNetViolationError);
    });
