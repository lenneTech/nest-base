...
/**
 * Story · AdminCrudModule controllers (`/hub/admin/roles`, `/hub/admin/policies`,
...
describe("Story · AdminCrudModule dev gating", () => {
  it("admin-crud controllers use @Public and the operational surface guard on every handler", () => {
    ...
    expect(src).toContain('@Controller("hub/admin/roles")');
    expect(src).toContain('@Controller("hub/admin/policies")');
    expect(src).toContain('@Controller("hub/admin/permissions")');
  ...
  it("RolesAdminPage loads roles via session bootstrap + fetchJson", () => {
    ...
    expect(src).toContain('fetchJson<RoleRecord[]>("/hub/admin/roles")');
