...
describe('Story · Better-Auth Social-Login', () => {
  function options(auth: ReturnType<typeof buildBetterAuth>): { socialProviders?: Record<string, unknown> } {
  ...
  it('wires google into Better-Auth options when configured', () => {
    ...
    expect(options(auth).socialProviders?.google).toEqual({ clientId: 'gid', clientSecret: 'gsecret' });
  });
  ...
  it('coexists with twoFactor and passkey wiring', () => {
    ...
    expect(typeof (auth.api as unknown as Record<string, unknown>).enableTwoFactor).toBe('function');
    expect(typeof (auth.api as unknown as Record<string, unknown>).generatePasskeyRegistrationOptions).toBe('function');
  });
});
