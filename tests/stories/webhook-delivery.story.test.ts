...
describe('Story · Webhook-Delivery', () => {
  describe('HMAC signature', () => {
    ...
    it('buildHmacSignatureHeader / verify round-trip', () => {
      ...
      expect(verifyHmacSignatureHeader('s3cr3t', '{"event":"foo"}', header, { now: 1700000000, toleranceSeconds: 300 })).toBe(true);
    });

    it('verify fails on a tampered body', () => {
      const header = buildHmacSignatureHeader('k', '1700000000', '{"x":1}');
      expect(verifyHmacSignatureHeader('k', '{"x":2}', header, { now: 1700000000, toleranceSeconds: 300 })).toBe(false);
    });

    it('verify rejects timestamps outside the tolerance window (replay protection)', () => {
      const header = buildHmacSignatureHeader('k', '1700000000', '{"x":1}');
      expect(verifyHmacSignatureHeader('k', '{"x":1}', header, { now: 1700001000, toleranceSeconds: 300 })).toBe(false);
    });

    it('verify rejects malformed headers', () => {
      expect(verifyHmacSignatureHeader('k', '{}', 'bogus', { now: 1700000000, toleranceSeconds: 300 })).toBe(false);
      expect(verifyHmacSignatureHeader('k', '{}', '', { now: 1700000000, toleranceSeconds: 300 })).toBe(false);
    });
  });
  ...
  describe('Retry policy', () => {
    ...
    it('shouldAutoDisable() flips at the configured failure threshold', () => {
      expect(shouldAutoDisable(WEBHOOK_RETRY_DEFAULTS.autoDisableAfter - 1, WEBHOOK_RETRY_DEFAULTS)).toBe(false);
      expect(shouldAutoDisable(WEBHOOK_RETRY_DEFAULTS.autoDisableAfter, WEBHOOK_RETRY_DEFAULTS)).toBe(true);
    });
  });
});
