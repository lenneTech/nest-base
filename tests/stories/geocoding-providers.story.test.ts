...
describe('Story · GeocodingProvider adapters', () => {
  function fakeHttp(responseBody: unknown): GeocodingHttpClient & { calls: Array<{ url: string; headers?: Record<string, string> }> } {
  ...
  describe('MapboxGeocodingProvider', () => {
    ...
    it('normalises the Mapbox feature shape', async () => {
      const http = fakeHttp({
        features: [
          { center: [13.405, 52.52], place_name: 'Berlin, Deutschland' },
        ],
      });
      const provider = new MapboxGeocodingProvider({ http, accessToken: 'pk.test' });
      const out = await provider.geocode('Berlin');
  ...
  describe('contract — every adapter implements GeocodingProvider', () => {
    ...
    it('Nominatim, Mapbox, Google, LocalStub all match the interface', () => {
      ...
      assertContract(new MapboxGeocodingProvider({ http: fakeHttp({ features: [] }), accessToken: 't' }));
      assertContract(new GoogleGeocodingProvider({ http: fakeHttp({ status: 'OK', results: [] }), apiKey: 'k' }));
