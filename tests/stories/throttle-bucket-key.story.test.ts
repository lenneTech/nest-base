...
describe("Story · Throttle bucket key", () => {
  ...
  describe("identity priority", () => {
    it("uses the API-Key ID when present (highest priority)", () => {
      const key = buildThrottleBucketKey(
        subject({ apiKeyId: "ak-1", userId: "u-1", ip: "10.0.0.1" }),
      );
  ...
  describe("route participation", () => {
    it("embeds method + path so different routes do not share buckets", () => {
      const a = buildThrottleBucketKey(
        subject({ method: "POST", path: "/projects", userId: "u-1" }),
      );
      const b = buildThrottleBucketKey(
        subject({ method: "GET", path: "/projects", userId: "u-1" }),
      );
