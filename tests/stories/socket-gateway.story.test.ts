...
function makeResolver(map: Record<string, { userId: string; tenantId: string; rules: Array<{ action: string; subject: string; conditions?: Record<string, unknown> }> }>): SessionResolver {
  return {
    async resolve(token) {
...
describe('Story · Socket Gateway', () => {
  describe('handshake()', () => {
    it('returns the session for a valid token', async () => {
      const gateway = new SocketGateway(
        new FakeServer(),
        makeResolver({
          'tok-1': { userId: 'u1', tenantId: 't1', rules: [{ action: 'read', subject: 'Project' }] },
        }),
      );
  ...
  describe('subscribe()', () => {
    ...
    it('throws PermissionDeniedError when the ability denies', async () => {
      ...
      const session = await gateway.handshake('tok-1');
      await expect(gateway.subscribe(socket, session, 'Project:item:abc')).rejects.toThrow(PermissionDeniedError);
    ...
    it('honors tenant-scoped channel conditions', async () => {
      ...
      await expect(gateway.subscribe(socket, session, 'Project:tenant:t2')).rejects.toThrow(PermissionDeniedError);
    });
  });
  ...
  describe('unsubscribe() / dispatch()', () => {
    it('unsubscribe() removes the socket from the room', async () => {
      ...
      const gateway = new SocketGateway(
        server,
        makeResolver({ 'tok-1': { userId: 'u1', tenantId: 't1', rules: [{ action: 'read', subject: 'Project' }] } }),
      );
      const session = await gateway.handshake('tok-1');
      await gateway.subscribe(socket, session, 'Project:item:abc');
