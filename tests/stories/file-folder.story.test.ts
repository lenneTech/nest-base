...
describe('Story · File/Folder CRUD', () => {
  ...
  describe('FileService', () => {
    ...
    it('listInFolder() filters by tenant + folder', async () => {
      ...
      await svc.create({
        tenantId: 't1', folderId, filename: 'a.png', mimeType: 'image/png',
        sizeBytes: 1, sha256: '0'.repeat(64), storageDriver: 's3', storageKey: 'k1', uploaderId: 'u1',
      });
      await svc.create({
        tenantId: 't1', folderId, filename: 'b.png', mimeType: 'image/png',
        sizeBytes: 1, sha256: '1'.repeat(64), storageDriver: 's3', storageKey: 'k2', uploaderId: 'u1',
      });
      await svc.create({
        tenantId: 't1', folderId: null, filename: 'c.png', mimeType: 'image/png',
        sizeBytes: 1, sha256: '2'.repeat(64), storageDriver: 's3', storageKey: 'k3', uploaderId: 'u1',
      });
    ...
    it('remove() deletes the file record', async () => {
      ...
      const file = await svc.create({
        tenantId: 't1', folderId: null, filename: 'gone.txt', mimeType: 'text/plain',
        sizeBytes: 1, sha256: '0'.repeat(64), storageDriver: 's3', storageKey: 'k', uploaderId: 'u1',
      });
