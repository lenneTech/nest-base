...
describe('Story · Cross-Resource Search', () => {
  function makeExecutor(table: string, hits: Array<{ id: string; rank: number; title?: string }>): ResourceSearchExecutor {
    return {
      table,
  ...
  it('honors the limit across the merged set', async () => {
    const svc = new CrossResourceSearchService([
      makeExecutor('a', [{ id: 'a1', rank: 0.9 }, { id: 'a2', rank: 0.8 }]),
      makeExecutor('b', [{ id: 'b1', rank: 0.7 }, { id: 'b2', rank: 0.6 }]),
    ]);
  ...
  it('accepts an `only` allowlist that restricts which executors run', async () => {
    const calls: string[] = [];
    const executors: ResourceSearchExecutor[] = [
      { table: 'projects', async search() { calls.push('projects'); return []; } },
      { table: 'files', async search() { calls.push('files'); return []; } },
    ];
    const svc = new CrossResourceSearchService(executors);
