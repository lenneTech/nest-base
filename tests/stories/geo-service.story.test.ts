...
describe('Story · GeoService', () => {
  function fakeProvider(name: string, result: GeocodingResult | null): GeocodingProvider & { calls: string[] } {
    const calls: string[] = [];
    return {
      name,
      calls,
      async geocode(query) { calls.push(query); return result; },
      async reverseGeocode() { return result; },
    };
  }

  function inMemoryCache(): GeocodingCacheStore & { records: Map<string, { payload: unknown; expiresAt: number }> } {
  ...
  describe('buildFindNearbyQuery()', () => {
    ...
    it('embeds the centre point as ST_MakePoint(lng, lat) (PostGIS axis order)', () => {
      const sql = buildFindNearbyQuery({ table: 'addresses', lat: 52.52, lng: 13.405, radiusMeters: 500 });
      expect(sql).toMatch(/ST_MakePoint\(13\.405,\s*52\.52\)/);
    });

    it('uses ::geography so the radius is in metres (not degrees)', () => {
      const sql = buildFindNearbyQuery({ table: 'addresses', lat: 52.52, lng: 13.405, radiusMeters: 100 });
      expect(sql).toContain('::geography');
    });

    it('rejects a non-positive radius', () => {
      expect(() => buildFindNearbyQuery({ table: 'addresses', lat: 0, lng: 0, radiusMeters: 0 })).toThrow(/radius/i);
    });
  });
