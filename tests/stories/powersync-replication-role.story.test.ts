...
describe('Story · PowerSync replication role + publication', () => {
  function readPowerSyncMigration(): string {
    ...
    const candidates = readdirSync(migrationsDir).filter((entry) => /powersync|replication/i.test(entry));
  ...
  it('publication includes the user table (sync target)', () => {
    const sql = readPowerSyncMigration();
    expect(sql).toMatch(/PUBLICATION\s+powersync[\s\S]*?(FOR\s+ALL\s+TABLES|FOR\s+TABLE[\s\S]*?users?)/i);
  });
