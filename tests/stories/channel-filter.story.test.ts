...
} from '../../src/core/realtime/channel-filter.js';
...
describe('Story · Permission-Aware Channel-Filter', () => {
  ...
  describe('broadcast()', () => {
    ...
    it('routes broadcasts to the matching channel only (no cross-channel leak)', () => {
      ...
      filter.broadcast('Project:item:p1', 'project.updated', { record: { id: 'p1', tenantId: 't1' } });
    ...
    it('rejects malformed channel names', () => {
      const filter = new ChannelFilter();
      expect(() =>
        filter.broadcast('not-a-channel', 'x', { record: {} }),
      ).toThrow(/malformed/);
    });
  });
});
