...
describe('Story · Realtime channel permission', () => {
  ...
  describe('canSubscribeToChannel()', () => {
    ...
    it('honors per-item conditions (tenant scope)', () => {
      ...
      expect(canSubscribeToChannel(ability, { subject: 'Project', scope: 'tenant', id: 't1' })).toBe(true);
      // Channel scoped to a different tenant — denied
      expect(canSubscribeToChannel(ability, { subject: 'Project', scope: 'tenant', id: 'other' })).toBe(false);
    });

    it('denies subscription on an empty rule set', () => {
      const ability = buildAbility([]);
      expect(canSubscribeToChannel(ability, { subject: 'Project', scope: 'item', id: 'x' })).toBe(false);
    });
  });
});
