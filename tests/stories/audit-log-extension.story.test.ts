...
describe('Story · Audit-Log-Extension', () => {
  ...
  describe('top-level shape', () => {
    ...
    it('rejects an unknown action (footgun guard)', () => {
      expect(() =>
        buildAuditLogEntry(input({ action: 'frobnicate' as never })),
      ).toThrow(AuditLogActionUnknownError);
    });
  });
  ...
  describe('determinism', () => {
    it('returns byte-identical entries for byte-identical inputs', () => {
      expect(JSON.stringify(buildAuditLogEntry(input()))).toBe(JSON.stringify(buildAuditLogEntry(input())));
    });
  });
});
