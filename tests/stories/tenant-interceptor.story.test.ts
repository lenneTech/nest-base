...
describe('Story · Tenant-Interceptor + RLS', () => {
  ...
  describe('TenantInterceptor', () => {
    function makeContext(headers: Record<string, string | string[]>, path = '/api/users'): ExecutionContext {
    ...
    it('rejects requests to a tenant-required path that lack the header', async () => {
      ...
      await expect(Promise.resolve(result).then((r) => lastValueFrom(r))).rejects.toThrow(/tenant header/i);
    });
