...
describe('Story · Postgres Storage Adapter', () => {
  function makeOps(): FileBlobOperations & { rows: Map<string, { mimeType: string; body: Uint8Array }> } {
  ...
  it('get() throws StorageObjectNotFoundError on miss', async () => {
    const adapter = new PostgresStorageAdapter(makeOps(), { baseUrl: 'http://localhost:3000/files' });
  ...
  it('rejects empty key on put', async () => {
    const adapter = new PostgresStorageAdapter(makeOps(), { baseUrl: 'http://localhost:3000/files' });
    await expect(adapter.put({ key: '', body: asBytes('x'), mimeType: 't/p' })).rejects.toThrow(/key/);
  });
