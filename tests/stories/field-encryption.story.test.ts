...
describe('Story · Field-Encryption (AES-256-GCM)', () => {
  ...
  describe('KEK rotation', () => {
    ...
    it('rejects KEKs that are not exactly 32 bytes', () => {
      expect(() => new FieldEncryptionService({ getKek: () => Buffer.alloc(31) }).encrypt('x')).toThrow(/32/);
      expect(() => new FieldEncryptionService({ getKek: () => Buffer.alloc(33) }).encrypt('x')).toThrow(/32/);
    });
  });
