...
describe("Story · project-rename planner", () => {
  ...
  it("does NOT throw when only package.json matches but the slug is still the old one", () => {
    ...
    expect(() =>
      planProjectRename({ files: partial, newName: "nest-server-template" }),
    ).not.toThrow();
  ...
  it("rejects an invalid kebab-name (would corrupt YAML / package.json)", () => {
    expect(() => planProjectRename({ files: fixture(), newName: "My App!" })).toThrow(/kebab-case/);
    expect(() => planProjectRename({ files: fixture(), newName: "" })).toThrow(/kebab-case/);
  });
