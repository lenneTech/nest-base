...
describe('Story · Permission-Tester UI', () => {
  ...
  describe('the form', () => {
    ...
    it('echoes submitted values back into the form (so the admin sees what they typed)', () => {
      const html = renderPermissionTesterPage(input({ submitted: { userId: 'u-42', tenantId: 't-7' } }));
  ...
  describe('result section', () => {
    ...
    it('renders the userId and tenantId of the report', () => {
      const html = renderPermissionTesterPage(input({ report: report({ userId: 'u-42', tenantId: 't-7' }) }));
  ...
  describe('XSS safety', () => {
    it('escapes user-controlled values from the form', () => {
      const html = renderPermissionTesterPage(input({ submitted: { userId: '<script>alert(1)</script>', tenantId: 't1' } }));
      expect(html).not.toContain('<script>alert(1)</script>');
      expect(html).toContain('&lt;script&gt;');
    });

    it('escapes user-controlled values from the report', () => {
      const html = renderPermissionTesterPage(input({ report: report({ userId: '<img src=x onerror=alert(1)>' }) }));
