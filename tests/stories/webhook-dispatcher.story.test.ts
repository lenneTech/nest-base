...
describe('Story · Webhook Dispatcher', () => {
  function setup(initialEndpoint?: Partial<WebhookEndpointSnapshot>) {
    ...
    const deliveries: Array<{ id: string; status: string; statusCode?: number; attemptCount: number }> = [];
  ...
  it('signs the body and POSTs to the endpoint URL', async () => {
    ...
    const dispatcher = new WebhookDispatcher({ http, endpointStore, deliveryStore, now: () => 1700000000 });
  ...
  it('marks delivery DELIVERED + resets consecutive_failures on 2xx', async () => {
    const { endpoint, endpointStore, deliveryStore, deliveries } = setup({ consecutiveFailures: 3 });
    const http: WebhookHttpClient = {
      async post() {
    ...
    const dispatcher = new WebhookDispatcher({ http, endpointStore, deliveryStore, now: () => 1700000000 });
    await dispatcher.dispatch({ endpointId: endpoint.id, eventId: 'evt-1', eventType: 'x', body: '{}' });
  ...
  it('on 5xx → increments consecutive_failures, status FAILED', async () => {
    const { endpoint, endpointStore, deliveryStore, deliveries } = setup({ consecutiveFailures: 2 });
    const http: WebhookHttpClient = {
      async post() {
  ...
  it('throws when the endpoint id is unknown', async () => {
    ...
    const http: WebhookHttpClient = { async post() { return { ok: true, status: 200 }; } };
    const dispatcher = new WebhookDispatcher({ http, endpointStore, deliveryStore, now: () => 1700000000 });
    await expect(
