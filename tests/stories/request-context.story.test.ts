...
describe("Story · Request-Context (W3C Trace Context)", () => {
  describe("parseTraceparent()", () => {
    ...
    it("returns null on malformed headers (length / hex)", () => {
      ...
      expect(
        parseTraceparent("00-not-hex-zzzzzzzzzzzzzzzzzzzzzzzzzzzzz-zzzzzzzzzzzzzzzz-01"),
      ).toBeNull();
    });
  ...
  describe("runWithRequestContext()", () => {
    ...
    it("isolates concurrent contexts", async () => {
      const a = runWithRequestContext(
        { requestId: "A", traceId: "a", parentId: "p", sampled: true },
        async () => {
      ...
      const b = runWithRequestContext(
        { requestId: "B", traceId: "b", parentId: "p", sampled: true },
        async () => {
  ...
  describe("RequestContextMiddleware (NestJS)", () => {
    ...
    beforeAll(async () => {
      ...
      class CtxModule {
        configure(consumer: {
          apply: (m: typeof RequestContextMiddleware) => { forRoutes: (r: string) => void };
        }): void {
