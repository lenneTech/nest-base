...
describe('Story · Upload Session (TUS state machine)', () => {
  ...
  describe('appendChunk()', () => {
    ...
    it('throws UploadOffsetMismatchError when the client offset does not match', async () => {
      ...
      await expect(mgr.appendChunk(session.id, 0, asBytes(' more'))).rejects.toThrow(UploadOffsetMismatchError);
    });
    ...
    it('throws when the chunk would exceed the declared upload length', async () => {
      ...
      await expect(mgr.appendChunk(session.id, 0, asBytes('too long'))).rejects.toThrow(UploadTooLargeError);
    });

    it('throws UploadSessionNotFoundError on an unknown id', async () => {
      const mgr = new UploadSessionManager(makeStorage(), { maxUploadBytes: 1024 });
      await expect(mgr.appendChunk('missing', 0, asBytes('x'))).rejects.toThrow(UploadSessionNotFoundError);
    });
  ...
  describe('get() / abort()', () => {
    ...
    it('abort() removes the session and rejects subsequent appends', async () => {
      ...
      await expect(mgr.appendChunk(session.id, 0, asBytes('x'))).rejects.toThrow(UploadSessionNotFoundError);
    });
